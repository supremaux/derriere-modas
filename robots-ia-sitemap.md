...
### O que foi alterado:
- A constante `siteUrl` foi alterada para `https://derriere-modas.vercel.app` (sem a barra final, para evitar caminhos duplicados como `//#collections`).
- Todos os links estruturados no catálogo e sitemap agora apontam perfeitamente para o seu deploy de produção na Vercel.
...
build: {
inlineStylesheets: 'auto', // ✅ Só isso - o Astro cuida do resto
},
});
```
